...
const Suits = () => {
  return (
    <div className='w-full grid grid-cols-4 max-lg:grid-cols-2 max-sm:grid-cols-1  gap-8 p-8 '>
      {suitsData.map((suit) => (
        <Card key={suit.id} className='overflow-hidden shadow-md shadow-gray-400'>
        <CardHeader className='bg-white'>
          <Image src={suit.image} alt={suit.name} width={200} height={280}/>
        </CardHeader>
        <CardContent className='flex flex-col gap-2 mt-4'>
        <CardTitle>{suit.name}</CardTitle>
        <CardDescription>{suit.description.slice(0, 60)}...</CardDescription>
